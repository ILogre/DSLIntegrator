...
import fr.unice.i3s.dslintegrator.domains.compovisu.service.{DDModel, DDHistory}
...
class DataCenter extends Service{
  ...
  def addResource(catalogName : String, uri: String, semantic: String, elements: (String, String)*) = {
    val model = DBHistory.models.get(catalogName).get
    ...
    DBHistory addModel new DBModel(model.name,newListCatalog)
...
case class addResource(catalogName : String, uri: String, semantic: String, elements: (String, String)*) extends Message{
  override val target: Model = DBHistory.models.get(catalogName).get
}
